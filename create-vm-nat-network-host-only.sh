...
function usage() {
    ...
    echo "--nat-network Name of NAT Network, default = NatNetwork"
    echo "--hostonlyif Name of host only interface, default = vboxnet0"
    exit 2
}

ARGUMENTS=$(getopt -a -n alphabet -o n:i:b:v:c:h --long vm-name:,image-path:,base-dir:,vrde-port:,cpus:,memory-size:,video-memory-size:,os-type:,root-disk-size:,nat-network:,hostonlyif: -- "$@")
VALID_ARGUMENTS=$?
...
if [ -z $NAT_NETWORK_NAME ]
then
    NAT_NETWORK_NAME="NatNetwork"
fi
if [ -z $HOST_ONLY_IF ]
then

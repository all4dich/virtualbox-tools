...
ARGUMENTS=$(getopt -a -n alphabet -o n:i:b:v:c:h --long vm-name:,image-path:,base-dir:,vrde-port:,cpus:,memory-size:,video-memory-size:,os-type:,root-disk-size:,nat-network: -- "$@")
VALID_ARGUMENTS=$?
